...
class Metadata:
    ...
    def __repr__(self):
        # print(f"Metadata __repr__ called for type: {self.type}, auto: {self.auto}, subtype: {self.subtype}")    
        # Improved REPR to show exactly what's inside
        if self.type == "list":
    ...
    #             if self.type == "list":
    #                 print(f"Resolving list value: {value}")
    #                 return_list_data = []
    #                 if len(value) > 0:
    ...
    def resolveValue(self, value=None):
        ...
        # Discovery Phase: If type is UNK, lock it in based on first value seen
        if self.type == "UNK":
            if isinstance(value, list):
                self.type = "list"
                self.subtype = Metadata(type_="UNK") # Create recursive metadata
            elif isinstance(value, bool) or CheckBool(value):
                self.type = "bool"
                ...
                self.type = "str"
            logger.info(f"Resolved UNK to {self.type}")

        # Processing Phase: Now that type is known (or was already known)
        if self.type == "list":
            # print(f"Resolving list value: {value}")
            # Ensure it's actually a list or parseable string
            working_list = value
            if isinstance(value, str):
                try: working_list = loads(value)
                except: pass 
            ...
            # Recursively resolve every item in the list using the subtype metadata
            return [self.subtype.resolveValue(item) for item in working_list]
